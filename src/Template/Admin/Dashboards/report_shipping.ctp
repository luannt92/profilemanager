...
$heads     = [
    RECEIVED   => [
        'title'   => '<span class="label">' . __('Report Information')
            . '</span>',
        'class'   => 'text-center',
        'colspan' => [
            __('Quantity'),
...
if ( ! empty($items)) {
    ...
    foreach ($items as $key => $item) {
        ...
        foreach ($item as $keyItem => $value) {
            $profit     = $this->Utility->formatAndCalcPrice($value['price'] + $value['feeShipping']);
            $priceOrders
                       = $this->Utility->formatAndCalcPrice($value['priceOrders']);
            $feeShipping
                = $this->Utility->formatAndCalcPrice($value['feeShipping']);
            $shipperCommission
                = $this->Utility->formatAndCalcPrice($value['shipperCommission']);
            $htmlTable .= "<td>{$value['quantity']}</td><td>{$priceOrders}</td><td>{$feeShipping}</td><td>{$profit}</td><td>{$shipperCommission}</td>";
        }
        $lastRow++;
        $htmlTable .= "</tr>";
    }
