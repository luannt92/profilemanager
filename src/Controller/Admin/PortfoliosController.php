 ...
 */
class PortfoliosController extends CommonController
{
    /**
    ...
    protected function _searchFiltersCondition($conditions, $params = [])
    {
        if ( ! empty($params['keyword'])) {
            $conditions['Portfolios.title LIKE'] = '%' . trim($params['keyword'])
                . '%';
        }

        if (isset($params['status']) && is_numeric($params['status'])) {
            $conditions['Portfolios.status'] = trim($params['status']);
        }
    ...
    protected function _setVarToView()
    {
        ...
        $optionType = Configure::read('type_Portfolios');
    ...
    public function index($conditions = [])
    {
        $this->setReturnUrl();
        $conditions = [
            'Portfolios.status IN' => [ACTIVE, DEACTIVE, TRASH],
        ];
        ...
        $items          = $this->paginate($this->Portfolios);
    ...
    public function add()
    {
        $item = $this->Portfolios->newEntity();

        if ($this->request->is('post')) {
            $data = $this->request->getData();
            if ($this->Portfolios->checkType($data['type'])
                || $data['status'] == DEACTIVE
            ) {
                $item          = $this->Portfolios->patchEntity($item, $data);
                $item->user_id = $this->Auth->user('id');
                if ( ! empty($data['title'])) {
                    $item->slug = $this->convertToSlug($data['title']);
                }

                if ($this->Portfolios->save($item)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Portfolios->checkType($data['type'], $id)
                || $data['status'] == DEACTIVE
            ) {
                $dataSave = $this->Portfolios->patchEntity($item, $data);

                if ($this->Portfolios->save($dataSave)) {
                    $this->Flash->success(__(COMMON_MSG_0001));
